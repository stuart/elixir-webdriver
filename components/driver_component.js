...
goog.string.compareVersions = function(a, b) {
  for(var c = 0, d = goog.string.trim(String(a)).split("."), e = goog.string.trim(String(b)).split("."), f = Math.max(d.length, e.length), g = 0;0 == c && g < f;g++) {
    ...
    do {
      var l = n.exec(h) || ["", "", ""], r = p.exec(k) || ["", "", ""];
      if(0 == l[0].length && 0 == r[0].length) {
        break
      }
      var c = 0 == l[1].length ? 0 : parseInt(l[1], 10), q = 0 == r[1].length ? 0 : parseInt(r[1], 10), c = goog.string.compareElements_(c, q) || goog.string.compareElements_(0 == l[2].length, 0 == r[2].length) || goog.string.compareElements_(l[2], r[2])
    }while(0 == c)
  }
  return c
};
...
Utils.type = function(a, b, c, d, e, f, g) {
  ...
  if(d && h && k && n) {
    ...
    Utils.shiftCount = 0;
    for(var n = c.toUpperCase(), p = 0;p < c.length;p++) {
      var l = c.charAt(p);
      if("\ue000" == l) {
        ...
        l ? m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_F12 : "," == l || "<" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_COMMA, q = l.charCodeAt(0)) : "." == l || ">" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_PERIOD, q = l.charCodeAt(0)) : "/" == l || "?" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_SLASH, q = c.charCodeAt(p)) : "`" == l || "~" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_BACK_QUOTE, q = l.charCodeAt(0)) : "{" == l || "[" == l ? 
        (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_OPEN_BRACKET, q = l.charCodeAt(0)) : "\\" == l || "|" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_BACK_SLASH, q = l.charCodeAt(0)) : "}" == l || "]" == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_CLOSE_BRACKET, q = l.charCodeAt(0)) : "'" == l || '"' == l ? (m = Components.interfaces.nsIDOMKeyEvent.DOM_VK_QUOTE, q = l.charCodeAt(0)) : (m = n.charCodeAt(p), q = c.charCodeAt(p));
        if(r) {
          ...
          l = m;
          if(32 <= q && 127 > q && (l = 0, !r && h && 32 < q)) {
            if(97 <= q && 122 >= q) {
              q = q + 65 - 97
            }else {
              var s = String.fromCharCode(q).replace(/([\[\\\.])/g, "\\$1"), s = "`1234567890-=[]\\;',./".search(s);
              0 <= s && (q = '~!@#$%^&*()_+{}|:"<>?'.charCodeAt(s))
            }
          }
          s = Utils.keyEvent(a, b, "keydown", m, 0, e, r || h, k, d, !1);
          Utils.keyEvent(a, b, "keypress", l, q, e, r || h, k, d, !s);
  ...
  try {
    ...
    if(c.width) {
      if("area" == a.tagName.toLowerCase()) {
        ...
        if("poly" == a.shape) {
          for(var n = Number(f[0]), p = Number(f[1]), e = n, g = p, d = 0;d < f.length / 2;d++) {
            var l = Number(f[2 * d]), r = Number(f[2 * d + 1]), n = Math.min(n, l), p = Math.min(p, r), e = Math.max(e, l), g = Math.max(g, r)
          }
          return{x:c.left + n, y:c.top + p, width:e - n, height:g - p}
        }
      }
    ...
    var q = Utils.getLocationViaAccessibilityInterface(a);
    q.x = c.left;
    q.y = c.top;
    return q
  }catch(m) {
...
Dispatcher.notImplemented = function(a, b) {
  b.sendError(Response.NOT_IMPLEMENTED, "Unsupported command", "text/plain")
};
Dispatcher.executeAs = function(a) {
  return function(b, c) {
    for(var d = {name:a, sessionId:{value:b.getAttribute("sessionId")}, parameters:JSON.parse(b.getBody() || "{}")}, e = b.getAttributeNames(), f;f = e.shift();) {
    ...
    Components.classes["@googlecode.com/webdriver/command-processor;1"].getService(Components.interfaces.nsICommandProcessor).execute(d, function(a) {
      JSON.parse(a).status != bot.ErrorCode.SUCCESS && c.setStatus(Response.INTERNAL_ERROR);
...
Dispatcher.prototype.init_ = function() {
  this.bind_("/config/drivers").on(Request.Method.POST, Dispatcher.notImplemented);
  this.bind_("/status").on(Request.Method.GET, Dispatcher.executeAs("getStatus"));
  this.bind_("/session").on(Request.Method.POST, Dispatcher.executeAs("newSession"));
  this.bind_("/session/:sessionId").on(Request.Method.GET, Dispatcher.executeAs("getSessionCapabilities")).on(Request.Method.DELETE, Dispatcher.executeAs("quit"));
  this.bind_("/session/:sessionId/window_handle").on(Request.Method.GET, Dispatcher.executeAs("getCurrentWindowHandle"));
  this.bind_("/session/:sessionId/window_handles").on(Request.Method.GET, Dispatcher.executeAs("getWindowHandles"));
  this.bind_("/session/:sessionId/speed").on(Request.Method.GET, Dispatcher.executeAs("getSpeed")).on(Request.Method.POST, Dispatcher.executeAs("setSpeed"));
  this.bind_("/session/:sessionId/timeouts").on(Request.Method.POST, Dispatcher.executeAs("setTimeout"));
  this.bind_("/session/:sessionId/timeouts/implicit_wait").on(Request.Method.POST, Dispatcher.executeAs("implicitlyWait"));
  this.bind_("/session/:sessionId/timeouts/async_script").on(Request.Method.POST, Dispatcher.executeAs("setScriptTimeout"));
  this.bind_("/session/:sessionId/url").on(Request.Method.GET, Dispatcher.executeAs("getCurrentUrl")).on(Request.Method.POST, Dispatcher.executeAs("get"));
  this.bind_("/session/:sessionId/accept_alert").on(Request.Method.POST, Dispatcher.executeAs("acceptAlert"));
  this.bind_("/session/:sessionId/dismiss_alert").on(Request.Method.POST, Dispatcher.executeAs("dismissAlert"));
  this.bind_("/session/:sessionId/alert_text").on(Request.Method.GET, Dispatcher.executeAs("getAlertText")).on(Request.Method.POST, Dispatcher.executeAs("setAlertValue"));
  this.bind_("/session/:sessionId/forward").on(Request.Method.POST, Dispatcher.executeAs("goForward"));
  this.bind_("/session/:sessionId/back").on(Request.Method.POST, Dispatcher.executeAs("goBack"));
  this.bind_("/session/:sessionId/refresh").on(Request.Method.POST, Dispatcher.executeAs("refresh"));
  this.bind_("/session/:sessionId/execute").on(Request.Method.POST, Dispatcher.executeAs("executeScript"));
  this.bind_("/session/:sessionId/execute_async").on(Request.Method.POST, Dispatcher.executeAs("executeAsyncScript"));
  this.bind_("/session/:sessionId/source").on(Request.Method.GET, Dispatcher.executeAs("getPageSource"));
  this.bind_("/session/:sessionId/title").on(Request.Method.GET, Dispatcher.executeAs("getTitle"));
  this.bind_("/session/:sessionId/element").on(Request.Method.POST, Dispatcher.executeAs("findElement"));
  this.bind_("/session/:sessionId/elements").on(Request.Method.POST, Dispatcher.executeAs("findElements"));
  this.bind_("/session/:sessionId/element/active").on(Request.Method.POST, Dispatcher.executeAs("getActiveElement"));
  this.bind_("/session/:sessionId/element/:id").on(Request.Method.GET, Dispatcher.notImplemented);
  this.bind_("/session/:sessionId/element/:id/element").on(Request.Method.POST, Dispatcher.executeAs("findChildElement"));
  this.bind_("/session/:sessionId/element/:id/elements").on(Request.Method.POST, Dispatcher.executeAs("findChildElements"));
  this.bind_("/session/:sessionId/element/:id/text").on(Request.Method.GET, Dispatcher.executeAs("getElementText"));
  this.bind_("/session/:sessionId/element/:id/submit").on(Request.Method.POST, Dispatcher.executeAs("submitElement"));
  this.bind_("/session/:sessionId/element/:id/value").on(Request.Method.POST, Dispatcher.executeAs("sendKeysToElement"));
  this.bind_("/session/:sessionId/element/:id/name").on(Request.Method.GET, Dispatcher.executeAs("getElementTagName"));
  this.bind_("/session/:sessionId/element/:id/clear").on(Request.Method.POST, Dispatcher.executeAs("clearElement"));
  this.bind_("/session/:sessionId/element/:id/selected").on(Request.Method.GET, Dispatcher.executeAs("isElementSelected"));
  this.bind_("/session/:sessionId/element/:id/enabled").on(Request.Method.GET, Dispatcher.executeAs("isElementEnabled"));
  this.bind_("/session/:sessionId/element/:id/displayed").on(Request.Method.GET, Dispatcher.executeAs("isElementDisplayed"));
  this.bind_("/session/:sessionId/element/:id/location").on(Request.Method.GET, Dispatcher.executeAs("getElementLocation"));
  this.bind_("/session/:sessionId/element/:id/location_in_view").on(Request.Method.GET, Dispatcher.executeAs("getElementLocationOnceScrolledIntoView"));
  this.bind_("/session/:sessionId/element/:id/size").on(Request.Method.GET, Dispatcher.executeAs("getElementSize"));
  this.bind_("/session/:sessionId/element/:id/css/:propertyName").on(Request.Method.GET, Dispatcher.executeAs("getElementValueOfCssProperty"));
  this.bind_("/session/:sessionId/element/:id/attribute/:name").on(Request.Method.GET, Dispatcher.executeAs("getElementAttribute"));
  this.bind_("/session/:sessionId/element/:id/equals/:other").on(Request.Method.GET, Dispatcher.executeAs("elementEquals"));
  this.bind_("/session/:sessionId/cookie").on(Request.Method.GET, Dispatcher.executeAs("getCookies")).on(Request.Method.POST, Dispatcher.executeAs("addCookie")).on(Request.Method.DELETE, Dispatcher.executeAs("deleteAllCookies"));
  this.bind_("/session/:sessionId/cookie/:name").on(Request.Method.DELETE, Dispatcher.executeAs("deleteCookie"));
  this.bind_("/session/:sessionId/frame").on(Request.Method.POST, Dispatcher.executeAs("switchToFrame"));
  this.bind_("/session/:sessionId/window").on(Request.Method.POST, Dispatcher.executeAs("switchToWindow")).on(Request.Method.DELETE, Dispatcher.executeAs("close"));
  this.bind_("/session/:sessionId/window/:windowHandle/size").on(Request.Method.POST, Dispatcher.executeAs("setWindowSize")).on(Request.Method.GET, Dispatcher.executeAs("getWindowSize"));
  this.bind_("/session/:sessionId/window/:windowHandle/position").on(Request.Method.POST, Dispatcher.executeAs("setWindowPosition")).on(Request.Method.GET, Dispatcher.executeAs("getWindowPosition"));
  this.bind_("/session/:sessionId/window/:windowHandle/maximize").on(Request.Method.POST, Dispatcher.executeAs("maximizeWindow"));
  this.bind_("/session/:sessionId/window/:windowHandle/restore").on(Request.Method.POST, Dispatcher.executeAs("restoreWindow"));
  this.bind_("/session/:sessionId/screenshot").on(Request.Method.GET, Dispatcher.executeAs("screenshot"));
  this.bind_("/session/:sessionId/ime/available_engines").on(Request.Method.GET, Dispatcher.executeAs("imeGetAvailableEngines"));
  this.bind_("/session/:sessionId/ime/active_engine").on(Request.Method.GET, Dispatcher.executeAs("imeGetActiveEngine"));
  this.bind_("/session/:sessionId/ime/activated").on(Request.Method.GET, Dispatcher.executeAs("imeIsActivated"));
  this.bind_("/session/:sessionId/ime/deactivate").on(Request.Method.POST, Dispatcher.executeAs("imeDeactivate"));
  this.bind_("/session/:sessionId/ime/activate").on(Request.Method.POST, Dispatcher.executeAs("imeActivateEngine"));
  this.bind_("/session/:sessionId/element/:id/click").on(Request.Method.POST, Dispatcher.executeAs("clickElement"));
  this.bind_("/session/:sessionId/moveto").on(Request.Method.POST, Dispatcher.executeAs("mouseMove"));
  this.bind_("/session/:sessionId/buttondown").on(Request.Method.POST, Dispatcher.executeAs("mouseDown"));
  this.bind_("/session/:sessionId/buttonup").on(Request.Method.POST, Dispatcher.executeAs("mouseUp"));
  this.bind_("/session/:sessionId/click").on(Request.Method.POST, Dispatcher.executeAs("mouseClick"));
  this.bind_("/session/:sessionId/doubleclick").on(Request.Method.POST, Dispatcher.executeAs("mouseDoubleClick"));
  this.bind_("/session/:sessionId/keys").on(Request.Method.POST, Dispatcher.executeAs("sendKeysToActiveElement"));
  this.bind_("/session/:sessionId/log").on(Request.Method.POST, Dispatcher.executeAs("getLog"));
  this.bind_("/session/:sessionId/log/types").on(Request.Method.GET, Dispatcher.executeAs("getAvailableLogTypes"));
  this.bind_("/session/:sessionId/browser_connection").on(Request.Method.GET, Dispatcher.executeAs("isOnline"));
  this.bind_("/session/:sessionId/application_cache/status").on(Request.Method.GET, Dispatcher.executeAs("getAppCacheStatus"));
  this.bind_("/extensions/firefox/quit").on(Request.Method.POST, Dispatcher.executeAs("quit"))
};
...
Dispatcher.prototype.dispatch = function(a, b) {
  var c = a.getPathInfo();
  if(0 != c.indexOf("/hub")) {
    b.sendError(Response.NOT_FOUND)
  }else {
    ...
    if(d) {
      try {
        ...
        fxdriver.logging.error(f), b.sendError(Response.INTERNAL_ERROR, JSON.stringify({status:bot.ErrorCode.UNKNOWN_ERROR, value:fxdriver.error.toJSON(f)}), "application/json")
      }
    }else {
      b.sendError(Response.NOT_FOUND, "Unrecognized command: " + a.getMethod() + " " + a.getPathInfo(), "text/plain")
    }
  }
};
...
Resource.prototype.handle = function(a, b) {
  ...
  var c = a.getMethod();
  c == Request.Method.OPTIONS ? (b.setHeader("Allow", this.getAllowedMethods_()), b.setStatus(Response.OK), b.setBody(""), b.commit()) : (c == Request.Method.HEAD && (c = Request.Method.GET), (c = this.handlers_[c]) ? c(a, b) : (b.setHeader("Allow", this.getAllowedMethods_()), b.setContentType("text/plain"), b.sendError(Response.METHOD_NOT_ALLOWED, 'Method "' + a.getMethod() + '" not allowed for command "' + this.path_ + '"')))
};
Resource.prototype.getAllowedMethods_ = function() {
  ...
  a.push(Request.Method.OPTIONS);
  Request.Method.GET in this.handlers_ && a.push(Request.Method.HEAD);
  return a.join(",")
};
bot.html5 = {};
...
bot.events.TouchEventFactory_.prototype.create = function(a, b) {
  ...
  var f = goog.dom.getOwnerDocument(a), g = goog.dom.getWindow(f), h = e(b.changedTouches), k = b.touches == b.changedTouches ? h : e(b.touches), n = b.targetTouches == b.changedTouches ? h : e(b.targetTouches), p;
  bot.events.BROKEN_TOUCH_API_ ? (p = f.createEvent("MouseEvents"), p.initMouseEvent(this.type_, this.bubbles_, this.cancelable_, g, 1, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, b.metaKey, 0, b.relatedTarget), p.touches = k, p.targetTouches = n, p.changedTouches = h, p.scale = b.scale, p.rotation = b.rotation) : (p = f.createEvent("TouchEvent"), goog.userAgent.product.ANDROID ? p.initTouchEvent(k, n, h, this.type_, g, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, 
  b.metaKey) : p.initTouchEvent(this.type_, this.bubbles_, this.cancelable_, g, 1, 0, 0, b.clientX, b.clientY, b.ctrlKey, b.altKey, b.shiftKey, b.metaKey, k, n, h, b.scale, b.rotation), p.relatedTarget = b.relatedTarget);
  return p
};
...
fxdriver.modals.dismissAlert = function(a) {
  ...
  c || (fxdriver.logging.info("No cancel button Falling back to the accept button"), c = fxdriver.modals.findButton_(b, "accept"));
  c.click();
  fxdriver.modals.clearFlag_(a)
};
...
fxdriver.modals.signalOpenModal = function(a, b) {
  ...
  if(c && c.response_) {
    ...
    var d = c.response_;
    "executeAsyncScript" != c.response_.name || c.response_.responseSent_ ? (d.value = b, d.send()) : fxdriver.modals.isModalPresent(function(a) {
...
fxdriver.screenshot.grab = function(a) {
  ...
  d.width = e;
  d.height = c;
  d.getContext("2d").drawWindow(a, 0, 0, e, c, "rgb(255,255,255)");
  return d
};
  ...
  if(!bot.userAgent.isProductVersion("3.5")) {
    ...
    var d = function() {
      ...
      return{single:function(c, d) {
        var k = a(b + " return results[0] || null;", d);
        d = fxdriver.moz.unwrap(d);
        return k.call(null, c, d)
      }, many:function(c, d) {
        var k = a(b + " return results;", d);
        d = fxdriver.moz.unwrap(d);
        return k.call(null, c, d)
      }}
    }();
...
FirefoxDriver.prototype.get = function(a, b) {
  var c = b.url, d = a.session.getWindow().location;
  (d = fxdriver.io.isLoadExpected(d, c)) && new WebLoadingListener(a.session.getBrowser(), function(b) {
  ...
  d || (fxdriver.logging.info("No load event expected"), a.send())
};
...
function injectAndExecuteScript(a, b, c, d) {
  ...
  if(e.designMode && "on" == e.designMode.toLowerCase()) {
    ...
    fxdriver.logging.info("Window in design mode, falling back to sandbox: " + e.designMode);
    var h = a.session.getWindow(), h = h.wrappedJSObject, k = new Components.utils.Sandbox(h);
    k.window = h;
    k.document = e.wrappedJSObject ? e.wrappedJSObject : e;
    k.navigator = h.navigator;
    k.__webdriverParams = g;
    try {
      var n = Components.utils.evalInSandbox("with(window) { var __webdriverFunc = function(){" + b.script + "};  __webdriverFunc.apply(null, __webdriverParams); }", k);
      a.value = Utils.wrapResult(n, e);
      a.send();
      return
    }catch(p) {
      throw new WebDriverError(bot.ErrorCode.JAVASCRIPT_ERROR, p);
    }
  }
  var l = this, r = function() {
    ...
    if(bot.userAgent.isProductVersion(4)) {
      for(var b = 0;b < g.length;b++) {
        if("object" === goog.typeOf(g[b])) {
          var d = Object.keys(g[b]), h;
          for(h in d) {
            void 0 == g[b].__exposedProps__ && (g[b].__exposedProps__ = {}), g[b].__exposedProps__[d[h]] = "rw"
          }
        }
      }
    }
    ...
    var k = function(b) {
      e.removeEventListener("webdriver-evaluate-response", k, !0);
      l.modalOpen || (b = fxdriver.moz.unwrap(e).getUserData("webdriver-evaluate-result"), a.value = Utils.wrapResult(b, e), a.status = e.getUserData("webdriver-evaluate-code"), e.setUserData("webdriver-evaluate-result", null, null), e.setUserData("webdriver-evaluate-code", null, null), a.send())
    };
    e.addEventListener("webdriver-evaluate-response", k, !0);
    b = e.createEvent("Events");
    b.initEvent("webdriver-evaluate", !0, !1);
    e.dispatchEvent(b)
  };
...
FirefoxDriver.annotateInvalidSelectorError_ = function(a, b) {
  ...
  try {
    ...
    fxdriver.logging.info("Converted the exception: " + c.name);
...
FirefoxDriver.prototype.findElementInternal_ = function(a, b, c, d, e) {
  ...
  try {
    ...
    var n = a.session.getImplicitWait();
    if(0 == n || goog.now() - e > n) {
...
FirefoxDriver.prototype.switchToFrame = function(a, b) {
  ...
  var e = null;
  if(d) {
    fxdriver.logging.info("Switching to default content (topmost frame)"), e = a.session.getBrowser().contentWindow
  }else {
    if(goog.isString(b.id)) {
      fxdriver.logging.info("Switching to frame with name or ID: " + b.id), e = bot.frame.findFrameByNameOrId(b.id, c)
    }else {
      if(goog.isNumber(b.id)) {
        fxdriver.logging.info("Switching to frame by index: " + b.id), e = bot.frame.findFrameByIndex(b.id, c)
      }else {
        if(goog.isObject(b.id) && "ELEMENT" in b.id) {
          fxdriver.logging.info("Switching to frame by element: " + b.id.ELEMENT);
          c = Utils.getElementAt(b.id.ELEMENT, c.document);
...
FirefoxDriver.prototype.isOnline = function(a, b) {
  a.value = bot.connection.isOnline(a.session.getBrowser().contentWindow);
  a.send()
};
...
function getElementFromLocation(a, b) {
  ...
  a.initialized ? (c = b.elementFromPoint(d, e), fxdriver.logging.info("Element from (" + d + "," + e + ") :" + c)) : (fxdriver.logging.info("Mouse coordinates were not set - using body"), c = b.getElementsByTagName("body")[0]);
...
FirefoxDriver.prototype.mouseMove = function(a, b) {
  ...
  if(this.enableNativeEvents) {
    a: {
      var e = this.enableNativeEvents, f = this.jsTimer, g = null;
      d.auxiliary ? (g = fxdriver.moz.unwrap(d.auxiliary), c = Utils.getLocationRelativeToWindowHandle(g)) : (g = getElementFromLocation(a.session.getMousePosition(), c), c = a.session.getMousePosition());
      c.x = Math.floor(c.x + d.x);
      c.y = Math.floor(c.y + d.y);
      var h;
      try {
        h = bot.dom.getInViewLocation(c, a.session.getWindow())
      }catch(k) {
        if(k.code == bot.ErrorCode.MOVE_TARGET_OUT_OF_BOUNDS) {
          a.sendError(new WebDriverError(bot.ErrorCode.MOVE_TARGET_OUT_OF_BOUNDS, "Given coordinates (" + c.x + ", " + c.y + ") are outside the document. Error: " + k));
          break a
        }else {
          throw k;
        }
      }
      var n = (d = a.session.isMousePressed()) ? c : h, p = Utils.getBrowserSpecificOffset(a.session.getBrowser()), l = n.x + p.x, n = n.y + p.y, r = Utils.getNativeMouse(), q = Utils.getNodeForNativeEvents(g);
      if(e && r && q) {
        var e = a.session.getMousePosition(), m = e.x + p.x, p = e.y + p.y;
        fxdriver.logging.info("Moving from (" + e.x + ", " + e.y + ") to (" + h.x + ", " + h.y + ")");
        r.mouseMove(q, m, p, l, n);
        Utils.waitForNativeEventsProcessing(g, Utils.getNativeEvents(), {wasUnloaded:!1}, f);
        a.session.setMousePosition(h.x, h.y);
        d && a.session.setMouseViewportOffset(c.x, c.y)
      }else {
        throw generateErrorForNativeEvents(e, r, q);
      }
    }
    a.send()
  }else {
    f = d.auxiliary || c, fxdriver.logging.info("Calling move with: " + d.x + ", " + d.y + ", " + f), f = this.mouse.move(f, d.x, d.y), this.sendResponseFromSyntheticMouse_(f, a)
  }
};
FirefoxDriver.prototype.mouseDown = function(a, b) {
  var c = a.session.getDocument();
  if(this.enableNativeEvents) {
    var c = getElementFromLocation(a.session.getMousePosition(), c), d = Utils.getNativeMouse(), e = Utils.getNodeForNativeEvents(c);
    ...
    c = fxdriver.events.buildCoordinates(b, c), fxdriver.logging.info("Calling down with: " + c.x + ", " + c.y + ", " + c.auxiliary), c = this.mouse.down(c), this.sendResponseFromSyntheticMouse_(c, a)
  }
};
FirefoxDriver.prototype.mouseUp = function(a, b) {
  var c = a.session.getDocument();
  if(this.enableNativeEvents) {
    var c = getElementFromLocation(a.session.getMousePosition(), c), d = Utils.getNativeMouse(), e = Utils.getNodeForNativeEvents(c);
    if(this.enableNativeEvents && d && e) {
      var f = a.session.getMousePosition(), g = f.x, h = f.y;
      a.session.isMousePressed() && (g = f.viewPortXOffset, h = f.viewPortYOffset, fxdriver.logging.info("Button pressed. Using coordiantes with viewport offset: " + g + ", " + h));
    ...
    c = fxdriver.events.buildCoordinates(b, c), fxdriver.logging.info("Calling up with: " + c.x + ", " + c.y + ", " + c.auxiliary), c = this.mouse.up(c), this.sendResponseFromSyntheticMouse_(c, a)
  }
};
...
FirefoxDriver.prototype.sendKeysToActiveElement = function(a, b) {
  ...
  bot.dom.isEditable(c) && void 0 !== c.value && goog.dom.selection.setCursorPosition(c, c.value.length);
...
bot.action.drag = function(a, b, c, d, e, f) {
  ...
  for(var h = 1;h <= d;h++) {
    var k = Math.floor(h * b / d), n = Math.floor(h * c / d), p = bot.dom.getClientRect(a), k = new goog.math.Coordinate(e.x + g.left + k - p.left, e.y + g.top + n - p.top);
    f.move(a, k)
  }
  f.releaseButton()
};
...
var WebElement = {elementEquals:function(a, b) {
  ...
  if(!n && this.enableNativeEvents && e && f && h && k) {
    ...
    p = p.y + l;
    try {
      var r = a.session.getMousePosition(), q = new goog.math.Coordinate(h, p), m = Utils.getBrowserSpecificOffset(a.session.getBrowser());
      e.mouseMove(f, r.x + m.x, r.y + m.y, q.x + m.x, q.y + m.y);
      var s = Utils.getPageUnloadedIndicator(d);
      e.click(f, q.x + m.x, q.y + m.y, 1);
      a.session.setMousePosition(q.x, q.y);
      Utils.waitForNativeEventsProcessing(d, Utils.getNativeEvents(), s, this.jsTimer);
      a.send();
      return
    }catch(t) {
      if(fxdriver.logging.info("Detected error when clicking: " + t.name), "NS_ERROR_NOT_IMPLEMENTED" != t.name) {
        throw new WebDriverError(bot.ErrorCode.INVALID_ELEMENT_STATE, t);
      }
    }
  }
...
WebElement.sendKeysToElement = function(a, b) {
  ...
  f != c || d != new XPCNativeWrapper(g) ? (fxdriver.logging.info("Need to switch focus"), e = !1, f.blur(), c.focus(), c.ownerDocument.defaultView.focus()) : fxdriver.logging.info("No need to switch focus");
  var h = c;
  ...
  this.jsTimer.setTimeout(function() {
    ...
    Utils.type(a.session.getDocument(), h, b.value.join(""), k.enableNativeEvents, k.jsTimer, !0);
    a.send()
  }, 0)
};
...
WebElement.getElementSize = function(a, b) {
  var c = Utils.getElementAt(b.id, a.session.getDocument()), c = Utils.getLocation(c);
...
WebElement.getElementLocationOnceScrolledIntoView = function(a, b) {
  ...
  c = Utils.getLocationOnceScrolledIntoView(c);
  a.value = {x:Math.round(c.x), y:Math.round(c.y)};
  a.send()
};
...
WebDriverServer.prototype.newDriver = function(a) {
  this.enableNativeEvents || (this.enableNativeEvents = Utils.useNativeEvents(), fxdriver.logging.info("Using native events: " + this.enableNativeEvents));
...
WebDriverServer.prototype.startListening = function(a) {
  a || (a = fxdriver.moz.getService("@mozilla.org/preferences-service;1", "nsIPrefBranch"), a = a.prefHasUserValue("webdriver_firefox_port") ? a.getIntPref("webdriver_firefox_port") : 7055);
  this.isListening || (this.server_.start(a), this.isListening = !0)
};
...
var nsISupports = Components.interfaces.nsISupports, CLASS_ID = Components.ID("{1C0E8D86-B661-40d0-AE3D-CA012FADF170}"), CLASS_NAME = "firefoxWebDriver", CONTRACT_ID = "@googlecode.com/webdriver/fxdriver;1", ServerFactory = {createInstance:function(a, b) {
  if(null != a) {
