...
fxdriver.profiler.log = function(a) {
  fxdriver.logging.log(fxdriver.logging.LogType.PROFILER, fxdriver.logging.LogLevel.INFO, a)
};
fxdriver.proxy = {};
...
fxdriver.proxy.pacConfig_ = function(a, b) {
  fxdriver.logging.info("Using a PAC file to connect to the network: " + b.proxyAutoconfigUrl);
...
fxdriver.proxy.configure_ = function(a) {
  if(a) {
    goog.isString(a) && (a = JSON.parse(a));
    var b = fxdriver.proxy.TYPES_[a.proxyType];
    if(b) {
...
fxdriver.proxy.configure = function(a) {
  try {
    ...
    fxdriver.logging.info("Unable to configure logging. " + b)
  }
};
...
wdSession.prototype.getWindow = function() {
  ...
  try {
    ...
    fxdriver.logging.error(c)
  }
  if(!a) {
    try {
      this.window_ && (a = this.window_.get())
    }catch(d) {
      fxdriver.logging.error(d)
    }
  }
  a && a.document || (fxdriver.logging.error("Lost DOM in window " + a), a = this.chromeWindow_.getBrowser().contentWindow, this.setWindow(a));
  return a
};
...
wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING = {webStorageEnabled:"dom.storage.enabled", applicationCacheEnabled:"browser.cache.offline.enable", databaseEnabled:"dom.indexedDB.enabled", locationContextEnabled:"geo.enabled", browserConnectionEnabled:"dom.network.enabled", acceptSslCerts:"webdriver_accept_untrusted_certs", nativeEvents:"webdriver_enable_native_events"};
...
wdSessionStoreService.prototype.configureCapabilities_ = function(a, b) {
  ...
  goog.object.forEach(a, function(a, e) {
    if(goog.isBoolean(a) && e in wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING) {
      var f = wdSessionStoreService.CAPABILITY_PREFERENCE_MAPPING[e];
      c.setBoolPref(f, a);
      fxdriver.logging.info("Setting capability " + e + " (" + f + ") to " + a);
...
DelayedCommand.prototype.execute = function(a) {
  "unstable" == loadStrategy_ || this.yieldedForBackgroundExecution_ || (this.yieldedForBackgroundExecution_ = !0, fxdriver.profiler.log({event:"YIELD_TO_PAGE_LOAD", startorend:"start"}));
  var b = this;
  ...
  if(this.loadGroup_.isPending()) {
    for(var a = !1, b = 0, c = this.loadGroup_.requests;c.hasMoreElements();) {
      ...
      try {
        ...
        fxdriver.logging.info("Ignoring non-nsIRequest: " + e);
        continue
      }
      e = !1;
      try {
    ...
    if(b && !a) {
      return fxdriver.logging.info("Ignoring pending about:document-onload-blocker request"), !0
    }
  }
    ...
    try {
      ...
      var b = (new Date).getTime() + this.response_.session.getImplicitWait(), c = this.command_.name, d = this.driver_[c] || WebElement[c], e = this.command_.parameters, f = goog.bind(d, this.driver_, this.response_, e), g = goog.bind(this.checkPreconditions_, this, d.preconditions, this.response_, e), h = function() {
        try {
          ...
          (new Date).getTime() < b ? DelayedCommand.execTimer.setTimeout(h, 100) : (d.isWebDriverError || (fxdriver.logging.error("Exception caught by driver: " + c + "(" + e + ")"), fxdriver.logging.error(d)), a.sendError(d))
        }
      };
      h()
    }catch(k) {
      k.isWebDriverError || (fxdriver.logging.error("Exception caught by driver: " + this.command_.name + "(" + this.command_.parameters + ")"), fxdriver.logging.error(k)), this.response_.sendError(k)
    }
  }else {
...
nsCommandProcessor.prototype.execute = function(a, b) {
  ...
  if("newSession" == c.name || "quit" == c.name || "getStatus" == c.name || "getWindowHandles" == c.name) {
    fxdriver.logging.info("Received command: " + c.name);
    try {
    ...
    if(g) {
      ...
      fxdriver.logging.info("Received command: " + c.name);
      if("deleteSession" == c.name || "getSessionCapabilities" == c.name || "switchToWindow" == c.name || "getLog" == c.name || "getAvailableLogTypes" == c.name) {
      ...
      if(g = k.fxdriver) {
        if(k.getBrowser().contentWindow) {
          if(g.modalOpen && "getAlertText" != c.name && "setAlertValue" != c.name && "acceptAlert" != c.name && "dismissAlert" != c.name) {
            k = g.modalOpen;
            switch(fxdriver.modals.getUnexpectedAlertBehaviour()) {
              case "accept":
            ...
            fxdriver.logging.error("Sending error from command " + c.name + " with alertText: " + k);
            d.sendError(new WebDriverError(bot.ErrorCode.MODAL_DIALOG_OPENED, "Modal dialog present", {alert:{text:k}}))
          }else {
            "function" != typeof g[c.name] && "function" != typeof WebElement[c.name] ? (d.sendError(new WebDriverError(bot.ErrorCode.UNKNOWN_COMMAND, "Unrecognised command: " + c.name)), fxdriver.logging.error("Unknown command: " + c.name)) : (d.startCommand(k), (new DelayedCommand(g, c, d)).execute(0))
          }
        }else {
...
nsCommandProcessor.prototype.newSession = function(a, b) {
  ...
  d ? (d = Components.classes["@googlecode.com/webdriver/wdsessionstoreservice;1"].getService(Components.interfaces.nsISupports).wrappedJSObject.createSession(a, b.desiredCapabilities, b.requiredCapabilities, d), d = d.wrappedJSObject, d.setChromeWindow(c), a.session = d, a.sessionId = d.getId(), fxdriver.logging.info("Created a new session with id: " + d.getId()), this.getSessionCapabilities(a)) : a.sendError(new WebDriverError(bot.ErrorCode.UNKNOWN_ERROR, "No drivers associated with the window"));
  a.send()
};
nsCommandProcessor.prototype.getSessionCapabilities = function(a) {
  ...
  a.value = {cssSelectorsEnabled:!0, browserName:"firefox", handlesAlerts:!0, javascriptEnabled:!0, nativeEvents:Utils.useNativeEvents(), platform:c.OS, rotatable:!1, takesScreenshot:!0, version:b.version};
