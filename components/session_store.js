...
bot.dom.getOverflowState = function(a) {
  ...
  for(a = c(a);a;a = c(a)) {
    var l = bot.dom.getClientRect(a), e = b(a), p = d.left >= l.left + l.width, l = d.top >= l.top + l.height;
    if(p && "hidden" == e.x || l && "hidden" == e.y) {
      return bot.dom.OverflowState.HIDDEN
    }
    if(p && "visible" != e.x || l && "visible" != e.y) {
...
bot.dom.appendVisibleTextLinesFromElement_ = function(a, b) {
  if(bot.dom.isElement(a, goog.dom.TagName.BR)) {
    ...
    var h = bot.dom.isShown(a), l = null, p = null;
    h && (l = bot.dom.getEffectiveStyle(a, "white-space"), p = bot.dom.getEffectiveStyle(a, "text-transform"));
    goog.array.forEach(a.childNodes, function(a) {
      a.nodeType == goog.dom.NodeType.TEXT && h ? bot.dom.appendVisibleTextLinesFromTextNode_(a, b, l, p) : bot.dom.isElement(a) && bot.dom.appendVisibleTextLinesFromElement_(a, b)
    });
    f = goog.array.peek(b) || "";
...
Utils.type = function(a, b, c, d, e, f, g) {
  ...
  var h = Utils.getNativeKeyboard(), l = Utils.getNodeForNativeEvents(b), p = Components.classes["@mozilla.org/thread-manager;1"];
  if(d && h && l && p) {
    ...
    for(var p = c.toUpperCase(), r = 0;r < c.length;r++) {
      ...
      if("\ue000" == k) {
        ...
        if(q) {
          ...
          s = Utils.keyEvent(a, b, "keydown", m, 0, e, q || h, l, d, !1);
          Utils.keyEvent(a, b, "keypress", k, n, e, q || h, l, d, !s);
...
Utils.keyEvent = function(a, b, c, d, e, f, g, h, l, p) {
  b = void 0 == p ? !1 : p;
  p = 0;
  h && (p |= 1);
  f && (p |= 2);
  g && (p |= 4);
  l && (p |= 8);
  return a.defaultView.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsIDOMWindowUtils).sendKeyEvent(c, d, e, p, b)
};
  ...
  try {
    ...
    if(c.width) {
      if("area" == a.tagName.toLowerCase()) {
        ...
        if("circle" == a.shape) {
          var l = Number(f[2]);
          return{x:c.left + Number(f[0]) - l, y:c.top + Number(f[1]) - l, width:2 * l, height:2 * l}
        }
        if("poly" == a.shape) {
          for(var p = Number(f[0]), r = Number(f[1]), e = p, g = r, d = 0;d < f.length / 2;d++) {
            var k = Number(f[2 * d]), q = Number(f[2 * d + 1]), p = Math.min(p, k), r = Math.min(r, q), e = Math.max(e, k), g = Math.max(g, q)
          }
          return{x:c.left + p, y:c.top + r, width:e - p, height:g - r}
        }
      }
    ...
    var n = Utils.getLocationViaAccessibilityInterface(a);
    n.x = c.left;
    n.y = c.top;
    return n
  }catch(m) {
...
Utils.waitForNativeEventsProcessing = function(a, b, c, d) {
  ...
  do {
    ...
    fxdriver.logging.info("Extra events processed: " + l + " Page Unloaded: " + c.wasUnloaded)
