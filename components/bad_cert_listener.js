...
function getPreferenceFromProfile(a, b) {
  var c = CC["@mozilla.org/preferences-service;1"].getService(CI.nsIPrefBranch);
  if(!c.prefHasUserValue(a)) {
    return fxdriver.logging.info(a + " not set; defaulting to " + b), b
  }
  c = c.getBoolPref(a);
  fxdriver.logging.info("Found preference for " + a + ": " + c);
  return c
}
...
WdCertOverrideService.prototype.certificateExpiredBit_ = function(a, b) {
  return 0 != (b & a.CERT_EXPIRED) ? (fxdriver.logging.info("Certificate expired."), this.ERROR_TIME) : 0
};
WdCertOverrideService.prototype.certificateIssuerUntrusted_ = function(a, b) {
  return 0 != (b & a.ISSUER_UNKNOWN) || 0 != (b & a.ISSUER_NOT_TRUSTED) || 0 != (b & a.CERT_NOT_TRUSTED) || 0 != (b & a.INVALID_CA) ? (fxdriver.logging.info("Certificate issuer unknown."), fxdriver.logging.info("Unknown: " + (a.ISSUER_UNKNOWN & b)), fxdriver.logging.info("Issuer not trusted: " + (a.ISSUER_NOT_TRUSTED & b)), fxdriver.logging.info("Cert not trusted: " + (a.CERT_NOT_TRUSTED & b)), fxdriver.logging.info("Invalid CA: " + (a.INVALID_CA & b)), this.ERROR_UNTRUSTED) : 0
};
WdCertOverrideService.prototype.certificateHostnameMismatch_ = function(a, b) {
  var c = RegExp("^" + a.commonName.replace("*", "[\\w|-]+") + "$", "i");
  return null === b.match(c) ? (fxdriver.logging.info("Host name mismatch: cert: " + a.commonName + " get: " + b), this.ERROR_MISMATCH) : 0
};
WdCertOverrideService.prototype.fillNeededBits = function(a, b) {
  ...
  fxdriver.logging.info("Certificate verification results: " + c);
  d |= this.certificateExpiredBit_(a, c);
  d |= this.certificateHostnameMismatch_(a, b);
  0 == d && (fxdriver.logging.info("Checking issuer since certificate has not expired or has a host name mismatch."), d |= this.certificateIssuerUntrusted_(a, c));
  fxdriver.logging.info("return_bits now: " + d);
  return d
};
WdCertOverrideService.prototype.hasMatchingOverride = function(a, b, c, d, e) {
  var f = !1;
  !0 === shouldAcceptUntrustedCerts() ? (fxdriver.logging.info("Allowing certificate from site: " + a + ":" + b), f = !0, e.value = !1, bot.userAgent.isProductVersion(19) ? (fxdriver.logging.info("Setting all Override Bits"), d.value = this.ERROR_UNTRUSTED | this.ERROR_MISMATCH | this.ERROR_TIME) : d.value = this.fillNeededBits(c, a), fxdriver.logging.info("Override Bits: " + d.value)) : f = this.origListener_.hasMatchingOverride(a, b, c, d, e);
  return f
};
